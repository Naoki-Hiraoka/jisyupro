...
namespace jisyupro
{
  ...
  protected:
    ...
    //オリジナル
    ros::Publisher mypub_;
    std::vector<float> myheight_;
...
namespace jisyupro
{
  ...
  void MultiPlaneExtraction::onInit()
  {
    ...
    //オリジナル
    double temp;
    pnh_->param("height0", temp, 0.0);
    myheight_.push_back(temp);
    pnh_->param("height1", temp, 0.1);
    myheight_.push_back(temp);
    pnh_->param("height2", temp, 0.2);
    myheight_.push_back(temp);
    pnh_->param("height3", temp, 0.3);
    myheight_.push_back(temp);
    pnh_->param("height4", temp, 0.4);
    myheight_.push_back(temp);
    ...
    ////////////////////////////////////////////////////////
    //オリジナル
    mypub_ = advertise<jisyupro::Int32Array>(*pnh_, "output_histgram", 1);
				     ...
				     const jsk_recognition_msgs::PolygonArray::ConstPtr& polygons)
  {
    ...
    //オリジナル
    jisyupro::Int32Array myoutmsg;
    myoutmsg.header=input->header;
	...
	result_set.insert(output_indices->indices[i]);
      ...
      //オリジナル
      if(output_indices->indices.size()!=0){
	pcl::ExtractIndices<pcl::PointXYZRGB> myextract_indices;
	pcl::PointCloud<pcl::PointXYZRGB>::Ptr myresult_cloud{new pcl::PointCloud<pcl::PointXYZRGB>};
	myextract_indices.setKeepOrganized(keep_organized_);
	myextract_indices.setInputCloud(nonplane_cloud);
	myextract_indices.setIndices(output_indices);
	myextract_indices.filter(*myresult_cloud);
	for(size_t i=0;i<4;i++){
	  pcl::ExtractPolygonalPrismData<pcl::PointXYZRGB> myprism_extract;
	  myprism_extract.setViewPoint(viewpoint[0], viewpoint[1], viewpoint[2]);
	  myprism_extract.setInputCloud(myresult_cloud);
	  myprism_extract.setHeightLimits(-myheight_[i+1], -myheight_[i]);
	  myprism_extract.setInputPlanarHull(hull_cloud);
	  pcl::PointIndices myoutput_indices;
	  myprism_extract.segment(myoutput_indices);
	  myoutmsg.int32s.push_back(myoutput_indices.indices.size());
	}
      }else{
	for(int i=0;i<4;i++){
	  myoutmsg.int32s.push_back(0);
	}
      }
    }
